...
class CPVirusMachine(object):

    def __init__(self, hosts, instructions, instruction_connections, semantics="AND", non_determinism = False):
        self.hosts = hosts
    ...
    def step(self):
        ...
        if self.current_instruction.origin_hosts:
            origin_hosts = self.current_instruction.origin_hosts
            objective_hosts = self.current_instruction.objective_hosts
            multipliers = self.current_instruction.multipliers
            next_instructions = self.next_instructions(self.current_instruction)
            semantics_following = semantics == "AND"
            for (origin_host, objective_host, multiplier) in zip(origin_hosts,objective_hosts, multipliers):
                if origin_host.viruses:
                    origin_host.viruses -= 1
                    objective_host.viruses += 1 * multiplier
                    semantics_following = (semantics_following or True) if (semantics == "OR") else semantics_following
                else:
                    semantics_following = (semantics_following and False) if (semantics == "AND") else semantics_following
            if semantics_following:
